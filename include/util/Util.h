...
public:
	...
	static float getAngle(Point pt, Position pt2, float yaw);
	static float distancia(Point pt, Point pt2);
	static float distancia(Point pt, Position pt2);
	static cv::Point arToCv(Point pt);
	static ar::Point cvToAr(cv::Point pt);
	static Point getDistXY(int dist, float grados);
	static float pxToMt(int px, Data dt);
	static ar::Point pxToMt(ar::Point pt, Data dt);
	static int mtToPx(float mt, Data dt);
	static ar::Point mtToPx(ar::Point pt, Data dt);
