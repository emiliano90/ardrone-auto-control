...
float Util::distancia(Point pt, Position pt2)
{
	return distancia(pt, Point(pt2.x, pt2.y));
}
...
float Util::getAngle(Point pt, Position pt2, float yaw)
{
	return getAngle(pt, Point(pt2.x, pt2.y), yaw);
}
...
ar::Point Util::cvToAr(cv::Point pt)
{
	return cv::Point(pt.x, pt.y);
}
float Util::pxToMt(int px, Data dt)
{
	return px * SIZE_X_CM / dt.imgSize.width;
}
ar::Point Util::pxToMt(ar::Point pt, Data dt)
{
	pt.x = pt.x * SIZE_X_CM / dt.imgSize.width;
	pt.y = pt.y * SIZE_X_CM / dt.imgSize.width;
	return pt;
}
int Util::mtToPx(float mt, Data dt)
{
	return mt * dt.imgSize.width / SIZE_X_CM;
}
ar::Point Util::mtToPx(ar::Point mt, Data dt)
{
	mt.x = mt.x * dt.imgSize.width / SIZE_X_CM;
	mt.y = mt.y * dt.imgSize.width / SIZE_X_CM;
	return mt;
}
