...
void UI::drawRobot(Mat &image, const Data &data)
{
	...
	pt.y = data.tPos.y + pt.y;
	pt.x = data.tPos.x + 2 + pt.x;
	...
	pt.x = data.tPos.x + pt.x;
	pt.y = data.tPos.y + 2 + pt.y;
	...
	pt2.x = data.tPos.x + pt2.x;
	pt2.y = data.tPos.y + pt2.y;
...
void UI::drawDestinations(Mat &image, std::vector<cv::Point2i> destinations)
{
	for (size_t i = 0; i < destinations.size(); i++)
	{

		line(image, cv::Point(destinations[i].x, destinations[i].y - 5),
				cv::Point(destinations[i].x, destinations[i].y + 5), CF_BLACK,
				1, 8, 0);

		line(image, cv::Point(destinations[i].x - 5, destinations[i].y),
				cv::Point(destinations[i].x + 5, destinations[i].y), CF_BLACK,
				1, 8, 0);
	}
}
...
void UI::drawNextDestination(cv::Mat &image,
		std::vector<cv::Point2i> destinations, int destination)
{

	line(image, cv::Point(destinations[destination].x, destinations[destination].y - 5),
			cv::Point(destinations[destination].x, destinations[destination].y + 5), CF_BLACK, 2, 8,
			0);

	line(image, cv::Point(destinations[destination].x - 5, destinations[destination].y),
			cv::Point(destinations[destination].x + 5, destinations[destination].y), CF_BLACK, 2, 8,
			0);
