...
ar::Point Environment::getRobotPositionInImage(std::vector<ar::Point> puntos, Data dt)
{
	ar::Point p(0, 0);
	...
	for (int i = 0; i < (int)grupos.size(); i++)
	{
		...
		if (grupos[i].size() != 0)
		{
			...
			p.x = (dt.imgSize.width / 2) + (p.x - (dt.imgSize.width / 2)) * (ALT_CAMERA - dt.copterValues.altitude) / ALT_CAMERA;
			p.y = (dt.imgSize.height / 2) + (p.y - (dt.imgSize.height / 2)) * (ALT_CAMERA - dt.copterValues.altitude) / ALT_CAMERA;

			//convierto de pixeles a mt la posicion
			p.x = Util::pxToMt(p.x, dt);
			p.y = Util::pxToMt(p.y, dt);
...
void Environment::addDestinationZ(int point){
	this->destinationsZ.push_back(point);
}
void Environment::deleteLastDestinationZ(){
	this->destinationsZ.pop_back();
}
const int Environment::getDestinationZ(unsigned int i){
	if (i >= this->destinationsZ.size())
	{
		return 0;
	}
	return this->destinationsZ[i];
}
const std::vector<int> Environment::getDestinationsZ(){
	return this->destinationsZ;
}
void Environment::clearDestinationsZ(){
	this->destinationsZ.clear();
