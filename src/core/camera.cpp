...
const int DESTINOSZ = 4;
const bool BUSCAR_DESTINOS = false;

const bool WINDOW_GRAFICOS = false;
const bool WINDOW_AMBIENTE = true;
const bool WINDOW_CAMARA = false;
const bool WINDOW_HSV = false;
...
//	env->addDestination(Point2i(320, 240));
	...
	while (!quit)
	{
		...
		ar::Point p = env->getVideo()->tracking(img, hsv);
		...
		if (p.x != -1)
		{
			...
			//convierto de pixeles a mt la posicion
			p.x = Util::pxToMt(p.x, threadAttr->data);
			p.y = Util::pxToMt(p.y, threadAttr->data);
			...
			env->addLastPosition(threadAttr->data.tPos);
			threadAttr->data.tLastPos.push_back(threadAttr->data.tPos);
	...
	//	cv::cvtColor(img, segmentated, COLOR_BGR2HSV);
		...
		{//210,160
			env->addDestination(Util::arToCv(Util::pxToMt(ar::Point(210, 160), threadAttr->data)));
			env->addDestination(Util::arToCv(Util::pxToMt(ar::Point(210, 320), threadAttr->data)));
			env->addDestination(Util::arToCv(Util::pxToMt(ar::Point(430, 320), threadAttr->data)));
			env->addDestination(Util::arToCv(Util::pxToMt(ar::Point(430, 160), threadAttr->data)));
		}
		//invento unos destinos:
		if((int)env->getDestinationsZ().size() == 0)
		{
			env->addDestinationZ(600);
			env->addDestinationZ(600);
			env->addDestinationZ(600);
			env->addDestinationZ(600);
		}
...
*/
		...
		vp_os_mutex_lock(&threadAttr->mutex2);
				//ar::Point p = env->getRobotPositionInImage(puntos, threadAttr->data);
...
/* ahora lo hago en getRobotPositionInImage
		//modifico la posicion segun la altura del robot
		p.x = (threadAttr->data.imgSize.width / 2) + (p.x - (threadAttr->data.imgSize.width / 2)) * (ALT_CAMERA - threadAttr->data.copterValues.altitude) / ALT_CAMERA;
		p.y = (threadAttr->data.imgSize.height / 2) + (p.y - (threadAttr->data.imgSize.height / 2)) * (ALT_CAMERA - threadAttr->data.copterValues.altitude) / ALT_CAMERA;

		//convierto de pixeles a cm la posicion
		p.x = Util::pxToMt(p.x, threadAttr->data);
		p.y = Util::pxToMt(p.y, threadAttr->data);
*/
...
*/
		...
		if(threadAttr->data.changeDestino)
		{
			nDestino++;
			nDestinoZ++;
			if (nDestino == env->getDestinations().size())
			{
				nDestino = 0;
				nDestinoZ = 0;
			}
			...
			msChangeDestination = ((seconds) * 1000 + useconds / 1000.0) + 0.5;

			threadAttr->data.changeDestino = false;
			gettimeofday(&hover_start, NULL);
		}
		if(env->getDestinations().size() != 0)
		{
			// dibuja una cruz en los destinos
			UI::drawDestinations(grayImage, env->getDestinations(), threadAttr->data);
			UI::drawNextDestination(grayImage, env->getDestinations(), nDestino, threadAttr->data);

			threadAttr->data.destino.x = env->getDestination(nDestino)->x;
			threadAttr->data.destino.y = env->getDestination(nDestino)->y;
		}
		if(env->getDestinationsZ().size() != 0)
		{
			threadAttr->data.destino.z = env->getDestinationZ(nDestinoZ);
		}


		// informacion en la pantalla.
		string line1 = format("SET-> Pitch: %.3f, Roll: %.3f, Yaw: %.3f, Z: %.3f, Dist: %d, D_x: %d, D_y: %d",
				threadAttr->data.copterSets.pitch, threadAttr->data.copterSets.roll,
				threadAttr->data.copterSets.yaw, threadAttr->data.copterSets.altitude,
				threadAttr->data.dist_destino.z, threadAttr->data.dist_destino.x, threadAttr->data.dist_destino.y);

		string line2 = format(
				"GET--> Pitch: %.3f , Roll: %.3f, Yaw: %.3f, Altitude: %.1f cm, Desired Yaw: %.3f",
		...
		string line5 = format("POSICION--> X: %.2f, Y: %.2f, Z: %d, DestZ: %d",
				threadAttr->data.tPos.x, threadAttr->data.tPos.y, threadAttr->data.tPos.z, threadAttr->data.destino.z);

		string line6 = format("tiempo chekpoint: %d", msChangeDestination);
		...
		UI::drawText(grayImage, line6, ar::Point(10, 110), CF_BLACK);
...
void callbackMouseDestinations(int event, int x, int y, int flags, void* data)
{
	...
	if (event == cv::EVENT_LBUTTONDOWN)
	{
		...
		nDestinoZ++;
		if (nDestinoZ == DESTINOSZ) nDestinoZ = 0;
	}
